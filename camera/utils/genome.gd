...
const DEFAULT_GENOME_LENGTH := 58
...
static func decode_genome(genome: Array) -> Dictionary:
	...
	out.attack_k = _lerp(0.1, 4.0, _gval(g, 26, 1.0))
	...
	out.energy_k = _lerp(0.1, 4.0, _gval(g, 30, 1.0))
	...
	out.consumption_k = _lerp(0.1, 4.0, _gval(g, 33, 1.0))
	...
	out.eating_desire_k = _lerp(0.1, 4.0, _gval(g, 37, 1.0))
	...
	out.health_k = _lerp(0.1, 4.0, _gval(g, 40, 1.0))
	...
	out.view_range_k = _lerp(0.1, 4.0, _gval(g, 55, 1.0))
